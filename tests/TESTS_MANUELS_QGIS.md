...
- [ ] **33.13 Comparer les produits** : la liste de gauche de la fiche s'ouvre sur une première entrée « ⊞ Comparer les produits » ; elle affiche les deux tableaux traduits de la documentation RVT — « Caractéristiques des visualisations » (8 colonnes) et « Quel produit pour quel type de forme » (6 colonnes) — avec les **quatorze** lignes dans l'ordre des cartes de l'étape 2 et la légende à quatre niveaux `-` / `o` / `+` / `++` (plus les deux formes étoilées). Les cases portent le symbole publié, coloré : vert foncé gras pour `++`, vert pour `+`, ocre pour `o`, gris pour `-`. **MNT, Densité et Couverture n'ont aucun verdict** : ligne entière de points pâles, infobulle « non évalué par la source » (Kokalj n'évalue que des visualisations de relief). Survoler un en-tête de colonne donne sa légende ; survoler une case la redonne — vérifier en particulier que **Complexité** dit bien « ++ très complexe » et non « excellent ». Sous chaque tableau, la ligne de source cite Kokalj 2025 et sa licence CC BY.
