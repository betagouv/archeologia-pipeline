...
def read_world_file(world_file_path: str) -> Tuple[Optional[float], Optional[float], Optional[float], Optional[float]]:
    ...
    try:
        ...
        if len(lines) >= 6:
            pixel_width = float(lines[0].strip())    # Taille pixel X
            row_rotation = float(lines[1].strip())   # Rotation Y (généralement 0)
            col_rotation = float(lines[2].strip())   # Rotation X (généralement 0)
            pixel_height = float(lines[3].strip())   # Taille pixel Y (généralement négatif)
            x_origin = float(lines[4].strip())       # X origine (coin supérieur gauche)
            y_origin = float(lines[5].strip())       # Y origine (coin supérieur gauche)
...
def extract_coordinates_from_filename(filename: str) -> Tuple[Optional[float], Optional[float]]:
    ...
    try:
        # Pattern pour les fichiers RVT: LHD_FXX_XXXX_YYYY_TYPE_A_LAMB93
        # où XXXX et YYYY représentent les indices de dalle par km (→ mètres en multipliant par 1000)
        pattern = r'LHD_FXX_(\d{4})_(\d{4})_'
        match = re.search(pattern, filename)
        
        if match:
            x_km = int(match.group(1))  # 0929 → 929
            y_km = int(match.group(2))  # 6613 → 6613
            
            # Conversion kilomètres → mètres (Lambert 93)
            x_anchor = x_km * 1000      # 929 → 929000
            y_anchor = y_km * 1000      # 6613 → 6613000
...
def extract_tile_coordinates(filename: str) -> Optional[Tuple[str, str]]:
    ...
    Args:
        filename (str): Nom du fichier (ex: "LHD_FXX_0929_6613_LDO_A_LAMB93_detections")
    
    Returns:
        Tuple[Optional[str], Optional[str]]: (x_coord, y_coord) ou (None, None) si erreur
    """
    try:
        # Pattern pour les fichiers RVT: LHD_FXX_XXXX_YYYY_TYPE...
        parts = filename.split('_')
        if len(parts) >= 4:
            x_coord = parts[2]  # XXXX
            y_coord = parts[3]  # YYYY
...
def calculate_neighbor_tile_keys(x_coord: str, y_coord: str) -> List[str]:
    ...
    Args:
        x_coord (str): Coordonnée X de la tuile (ex: "0929")
        y_coord (str): Coordonnée Y de la tuile (ex: "6613")
    
    Returns:
        List[str]: Liste des clés de tuiles (format "XXXX_YYYY")
    """
    try:
        x_int = int(x_coord)
        y_int = int(y_coord)
        ...
        # Générer les 9 tuiles (8 voisins + centre)
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                neighbor_x = x_int + dx
                neighbor_y = y_int + dy
                # Formater avec des zéros en préfixe pour maintenir 4 chiffres
        ...
        return [f"{x_coord}_{y_coord}"]  # Fallback: seulement la tuile courante
...
def load_iou_threshold_from_config() -> float:
    ...
    Returns:
        float: Seuil IoU (défaut: 0.3 si non trouvé)
    """
    try:
        from pathlib import Path
        import json
...
def load_model_name_from_config() -> str:
    """Charge le nom du modèle de détection depuis config.json.
...
def _polygon_iou(poly_a, poly_b) -> Optional[float]:
    """
    Calcule l'Intersection over Union (IoU) entre deux polygones shapely.

    Args:
        poly_a (shapely.geometry.Polygon): Premier polygone
        poly_b (shapely.geometry.Polygon): Deuxième polygone

    Returns:
        Optional[float]: IoU dans [0,1] ou None si calcul impossible
    """
    try:
...
def _deduplicate_shapefiles_by_tile_extents(
    ...
    try:
        ...
        for shp in shapefile_paths:
            ...
            try:
                ...
                if "confidence" not in gdf.columns:
                    gdf["confidence"] = -1.0
                try:
                    gdf["confidence"] = gdf["confidence"].apply(lambda v: float(v) if v is not None and str(v).strip() != "" else float('nan'))
        ...
        # 1) Supprimer les détections qui ne sont dans l'emprise d'aucune dalle
        # (c.-à-d. hors de toutes les emprises JPG/JGW connues)
        try:
            ...
            if tile_union is not None:
                ...
                if len(outside_idx) > 0:
                    ...
                    logger.info(f"Déduplication finale: {len(outside_idx)} suppression(s) hors emprise de toutes les dalles")
        except Exception as e:
            ...
            for i_pos in range(len(kept_indices)):
                ...
                for j_pos in range(i_pos + 1, len(kept_indices)):
                    ...
                    if conf_i >= conf_j:
                        ...
                        break
            if local_dropped:
                logger.info(f"Déduplication finale: {len(local_dropped)} suppression(s) dans l'emprise {tile_name} (IoU>{iou_threshold})")
...
def deduplicate_shapefiles_final(
    ...
    """Déduplication finale à exécuter en fin de pipeline.

    Règle:
    - Pour chaque emprise de dalle (calculée à partir des .jgw/.jpg),
      on récupère toutes les détections (toutes classes confondues) dans l'emprise.
    - Si IoU > iou_threshold entre 2 détections, on supprime celle avec la plus faible confidence
      (confidence absente => -1, donc supprimée en priorité).
    """
...
def create_shapefile_from_detections(labels_dir: str, output_shapefile: str, 
                                   tif_transform_data: dict = None,
                                   crs: str = "EPSG:2154",
                                   temp_dir: str = None,
                                   class_names: dict = None) -> bool:
    """
    ...
    try:
        ...
        # Dictionnaire par défaut des noms de classes si non fourni
        ...
        for label_file in labels_path.glob("*.txt"):
            ...
            if tif_key:
                ...
                logger.debug(f"Utilisation des données TIF pour {base_name} (clé: {tif_key}): px_w={pixel_width}, px_h={pixel_height}, x_orig={x_origin}, y_orig={y_origin}")
                ...
                if temp_path.exists():
                    ...
                    for search_pattern in search_patterns:
                        ...
                        if matching_files:
                            ...
                            if any(pattern in search_pattern for pattern in ["_PTS_C_LAMB93_IGN69_LD.tif", "_PTS_C_LAMB93_IGN69_MNT.tif", "_LD_A_LAMB93.tif", "_MNT_A_0M50_LAMB93_IGN69.tif"]):
                                    ...
                                    try:
                                        ...
                                        if det_coords == tif_coords:
                                            ...
                                            if "_PTS_C_LAMB93_IGN69" in tif_file.name:
                                                logger.info(f"TIF avec marges trouvé pour {base_name}: {tif_file.name} (coordonnées: {det_coords})")
                                            else:
                                                logger.info(f"TIF rogné trouvé pour {base_name}: {tif_file.name} (coordonnées: {det_coords})")
                                            break
                    ...
                    if potential_tif_files:
                        ...
                        try:
                            ...
                            with rasterio.open(str(tif_file)) as src:
                                ...
                                logger.info(f"✅ Utilisation du TIF Temp {tif_file.name}: dimensions={src.width}x{src.height}, px_w={pixel_width}, px_h={pixel_height}, x_orig={x_origin}, y_orig={y_origin}")
                        except Exception as e:
                            ...
                            try:
                                ...
                                if gdalinfo:
                                    result = subprocess.run([gdalinfo, '-json', str(tif_file)],
                                    ...
                                    if len(gt) >= 6:
                                        ...
                                        logger.info(f"✅ Fallback gdalinfo pour {tif_file.name}: px_w={pixel_width}, px_h={pixel_height}, x_orig={x_origin}, y_orig={y_origin}")
                                    else:
            ...
            if not all(v is not None for v in [pixel_width, pixel_height, x_origin, y_origin]):
                ...
                logger.warning(f"Données de géoréférencement non disponibles pour {base_name}, fallback approximatif basé sur le nom")
            ...
            if json_file.exists():
                try:
                    ...
                    logger.info(f"Données de confiance chargées depuis {json_file.name}: {len(confidence_data)} détections avec confiance")
                ...
                with open(label_file, "r") as f:
                    for line_num, line in enumerate(f, 1):
                        ...
                        if jpg_path.exists():
                            ...
                            logger.warning(f"Image JPG non trouvée: {jpg_path}, utilisation des dimensions par défaut")
                            logger.debug(f"Recherche JPG - base_name: {base_name}, jpg_name: {jpg_name}, jpg_path: {jpg_path}")
                            ...
                            if jgw_path.exists():
                                ...
                                if all(val is not None for val in [pw, ph, xo, yo]):
                                    ...
                                    if jpg_key is None or jpg_key not in jgw_logged_for_jpg:
                                        logger.info(f"Utilisation du .jgw pour {jpg_path.name}: px_w={pixel_width}, px_h={pixel_height}, x_orig={x_origin}, y_orig={y_origin}")
                        ...
                        if x_coord is None or y_coord is None:
                            logger.warning(f"Impossible d'extraire les coordonnées de tuile de {base_name}, déduplication globale utilisée")
                            tile_key = "global"
                        else:
        ...
        for class_id, detections in data_by_class.items():
            ...
            try:
                ...
                if "confidence" in gdf.columns:
                    # confidence peut être None
                    gdf["confidence"] = gdf["confidence"].fillna(-1.0)
                    try:
                        gdf["confidence"] = gdf["confidence"].astype(float)
                    except Exception:
                        # Dernier recours: convertir via string
                        gdf["confidence"] = gdf["confidence"].astype(str)

                if len(gdf) == 0:
                    logger.warning(f"Aucune géométrie valide après nettoyage pour la classe '{class_name}' - shapefile non écrit")
                    continue
                ...
                try:
                    ...
                    try:
                        ...
                        for det in detections:
                            ...
                            try:
                                # shapely geometry → __geo_interface__
                        ...
                        try:
                            ...
                            if gdf.crs is not None:
                                # gdf.crs peut être un pyproj.CRS; utiliser to_string si dispo
                                crs_str = getattr(gdf.crs, 'to_string', lambda: str(gdf.crs))()
                            if not crs_str:
                                crs_str = crs  # utiliser le CRS attendu par défaut
                            if crs_str:
        ...
        try:
            ...
            try:
                ...
                # En cas de format inattendu, on laisse QGIS déduire le CRS
                pass
            ...
            try:
                style_path = Path(__file__).parent.parent / 'models' / 'style_detections.qml'
            ...
            def _apply_confidence_symbology(maplayer_el) -> None:
                ...
                """
                try:
                    # Palettes à 5 niveaux (1 par intervalle) : du plus faible (0) au plus fort (4)
                    palettes = [
                        # 0) Jaune → Rouge
            ...
            for shp_idx, shp in enumerate(created_shapefiles):
                ...
                # Stocker l'index de palette sur la couche (lecture côté _apply_confidence_symbology)
                ml.set('dataSourcePaletteIndex', str(shp_idx % 6))
                SubElement(ml, 'id').text = layer_id
                SubElement(ml, 'layername').text = layer_id
                SubElement(ml, 'datasource').text = str(shp_path.resolve())
                SubElement(ml, 'provider').text = 'ogr'
                # Associer explicitement le CRS à la couche vecteur
                try:
                ...
                # model_name (non éditable, simple champ texte)
                SubElement(fcfg, 'field', attrib={'name': 'model_name'})
                # corr_pred (éditable, NULL autorisé) — aliasé en "correction_prediction"
                ...
                # Ordre du formulaire: model_name, model_pred, validation, corr_pred, confidence
                ...
                # Appliquer une symbologie basée sur conf_bin (low/mid/high)
                _apply_confidence_symbology(ml)

                # Conserver uniquement les paramètres non-symbologie du QML si disponibles
            ...
            for tif_path in tif_files:
                ...
                SubElement(ml_raster, 'datasource').text = str(tif_path.resolve())
                SubElement(ml_raster, 'provider').text = 'gdal'
                # Associer explicitement le CRS à la couche raster
                try:
...
# === PARAMÈTRES ===
labels_dir = r"C:\OSGeo4W\IA\YOLO\runs_HD\detect\predict-GARD\labels"        # Dossier contenant les fichiers .txt
output_file = r"C:\OSGeo4W\IA\YOLO\predictions_points_GARD_shp"

pixel_size = 0.5                # 0.5 mètre par pixel
img_size = 2000                 # 2000 x 2000 pixels

if __name__ == "__main__":
    # Ce module est destiné à être importé par le pipeline.
    # L'exécution directe n'est pas supportée ici.
    import sys
